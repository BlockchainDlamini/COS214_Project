...
{
    std::shared_ptr<floorComponent> floor = std::make_shared<floorComponent>(myFloor(0));

    for (int i = 0; i < 4; i++)
    {
        std::shared_ptr<floorComponent> tile = std::make_shared<floorComponent>(myFloor(1+i));
        std::shared_ptr<floorComponent> tbl = std::make_shared<floorComponent>(table(1+i));
        tile->add(tbl);
        floor->add(tile);
        std::shared_ptr<floorComponent> tile2 = std::make_shared<floorComponent>(myFloor(2+i));
        std::shared_ptr<floorComponent> tbl2 = std::make_shared<floorComponent>(table(2+i));
        tile2->add(tbl2);
        floor->add(tile2);
        std::shared_ptr<floorComponent> tile3 = std::make_shared<floorComponent>(myFloor(3+i));
        std::shared_ptr<floorComponent> tbl3 = std::make_shared<floorComponent>(table(3+i));
        tile3->add(tbl3);
        floor->add(tile3);
        std::shared_ptr<floorComponent> tile4 = std::make_shared<floorComponent>(myFloor(4+i));
